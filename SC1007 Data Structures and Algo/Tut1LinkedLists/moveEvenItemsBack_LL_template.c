...
void moveEvenItemsToBack(LinkedList *ll)
{
	ListNode *cur, *tmp, *tmphead, *pre; //cursor
	cur = ll->head; //points to last
	tmphead = NULL;

		while (cur != NULL || ll->size != 0) //check if empty list
		{
			if ( (cur->item) % 2 == 0) //check if even
			{
				if (tmphead == NULL)
				{
					tmphead = malloc(sizeof(ListNode));
					tmp = tmphead
				}
				else
				{
