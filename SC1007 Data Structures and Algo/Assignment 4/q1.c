...
int HashInsert(int key, HashSlot hashTable[])
{
    ...
    int i = hash1(key), probe = hash2(key), comparison = 0, count = 0, temp = -1;
    if (hashTable[i].indicator == EMPTY){
        ...
        while (hashTable[i].indicator != EMPTY && count < TABLESIZE ){
            ...
            if (hashTable[i].indicator == DELETED && temp == -1){
                temp = i;
            }
            count++;
            i = ((i+probe)%TABLESIZE);
        ...
        if (hashTable[temp].indicator == DELETED){
...
{
    //Write your code here
    int i = hash1(key), probe = hash2(key), comparison = 0, count = 0;
    if (hashTable[i].indicator == EMPTY){
        return -1;
    }
    else{
