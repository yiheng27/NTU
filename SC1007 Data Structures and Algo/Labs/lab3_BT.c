...
int hasGreatGrandchild(BTNode *node){
    int height,l,r;
    height = 0; //calculate height, upwards --> post-order
    if (node==NULL){
        return -1;
    }
    ...
    if (height > 2){    //0-->no child, 1->child, 2->grandchild, 3->greatgrandchild
        printf(" %i", node->item);
    }
    return height;
}
