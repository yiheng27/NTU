/*1. (sweepSpace) Write two versions of a C function that remove all the blank spaces in a string.
...
char *sweepSpace1(char *str)//correct
{
   int j=0, len;
    len=strlen(str);
    for (int i=0; i<len;i++)
...
char *sweepSpace2(char *str)//wrong
{
   int j = 0,i=0;
   while (*(str+j) != '\0')
   {
